...
#MONGOHOST='localhost'
class data():
    def __init__(self,mongo_host='localhost',port=27017):
        self.MONGOHOST = mongo_host
        self.MONGOPORT = port
    def find(self, db=None, col=None, query=None, callback=None, 
    ...
    def distinct(self,db=None, col=None, distinct_key=None, query=None, callback=None):
        con = Connection(self.MONGOHOST,self.MONGOPORT)
    ...
    def group(self,db=None, col=None, key=None,variable=None,query=None, callback=None):
        """Perform group by aggregations from a specific mongoDB db and collection
            ...
            allow options for JSON/JSONP
        """
        con = Connection(self.MONGOHOST,self.MONGOPORT, replicaset='cybercom', read_preference=ReadPreference.SECONDARY)
        # if db is set create db object, else show db names
        if db:
            db = con[db]
        else:
        ...
        try:
            ...
            dump_out = self.multikeysort(results, sortlist)
