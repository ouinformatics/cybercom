...
TARGET_PROJECTION="+proj=merc +lon_0=0 +k=1 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"
SOURCE_PROJECTION="+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"

def mkGeoJSONPoint(obj,latkey,lonkey,attributes=False, t_srs=TARGET_PROJECTION, s_srs=SOURCE_PROJECTION):
    FC = []
    for item in obj:
        x = item.pop(lonkey)
        y = item.pop(latkey)
